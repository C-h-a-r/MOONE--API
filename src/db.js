...
async function giveCredits(userid, amount, db) {
  await db.query("UPDATE user:$userid SET credits += $amount", {
    userid: userid,
    amount: amount,
  });
}
...
async function finishGame(gameid, winnerid, db) {
  const [game] = await db.query("SELECT pot, players FROM game:$gameid", {
    gameid: gameid,
  });
  let pot = game.pot;
...
async function rollbackGames(db) {
  // probably only want to do this manually, maybe on each server restart idk
  ...
  games.forEach(async (game) => {
    ...
    game.players.forEach(async (playerid) => {
      await db.query("UPDATE $playerid SET credits += $bet", {
        playerid: playerid,
        bet: bet,
      });
    });
